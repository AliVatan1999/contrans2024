...
class contrans:
    ...
    def make_headers(self, email='wbz5tq@virginia.edu'):
    ...
    def make_cand_table(self, members):
        ...
        members['name2'] = [y.strip() + ' (' + z.strip() + ')' for y, z in zip(members['lastname'], members['partyletter'])]

        cands = pd.read_csv('cands22.txt', quotechar="|", header=None)
        ...
        crosswalk = pd.merge(members, cands, left_on=['name2', 'DistIDRunFor'], right_on=['name2', 'DistIDRunFor'], how='left')
        return crosswalk
    ...
    def query_mongo_searchengine(self, collection, keytosearch, searchterms, columns={}):
        ...
        cursor = collection.find({'$text': {'$search': searchterms, '$caseSensitive': False}}, columns)
        result_dumps = dumps(cursor)
        result_loads = loads(result_dumps)
        result_df = pd.DataFrame.from_records(result_loads)
        return result_df
    ...
    def make_members_df(self, members, ideology, engine):
        members_df = pd.merge(members, ideology, left_on='bioguideId', right_on='bioguide_id', how='left')
